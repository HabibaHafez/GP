...
import 'package:techmate/AdminUser/MentorsList.dart'; // Import MentorListScreen
...
class _AdminHomeScreenState extends State<AdminHomeScreen> {
  ...
  final FetchAllInternshipsService _fetchAllInternshipsService =
  FetchAllInternshipsService();
  final SearchInternshipsService _searchInternshipsService =
  SearchInternshipsService();
  final DeleteInternshipService _deleteInternshipService =
  ...
  void _fetchInternships() async {
    ...
    try {
      ...
      await _fetchAllInternshipsService.fetchAllInternships();
  ...
  void _filterInternships() async {
    ...
    try {
      List<Internship> internships =
      await _searchInternshipsService.searchInternships(query);
      setState(() {
        _filteredInternships = internships;
        _isLoading = false;
      });
  ...
  void _deleteInternship(int internId) async {
    ...
    try {
      ...
      if (success) {
        setState(() {
          _internships
              .removeWhere((internship) => internship.internId == internId);
  ...
  void _navigateToMentorListScreen() {
    Navigator.push(
      context,
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const <BottomNavigationBarItem>[
          ...
          BottomNavigationBarItem(
            icon: Icon(Icons.group), // Changed icon here
            label: 'Mentors List',
          ),
          BottomNavigationBarItem(
        ...
        onTap: (index) {
          if (index == 1) {
            _navigateToMentorListScreen(); // Navigate to MentorListScreen
          } else if (index == 2) {
            _logout();
          }
        },
