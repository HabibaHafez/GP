...
class _MentorProfileScreenState extends State<MentorProfileScreen> {
  ...
  void _loadUserImage() async {
    if (_nationalId != null) {
      String? imageUrl =
      await _profileApiService.fetchUserImageUrl(_nationalId!);
      setState(() {
        if (imageUrl != null && imageUrl.isNotEmpty) {
          _imageUrl = imageUrl;
        }
  ...
  Future<void> _uploadImage(File image) async {
    if (_image != null && _nationalId != null) {
      bool success =
      await _profileApiService.uploadUserImage(_nationalId!, image);
      if (success) {
        _loadUserImage();
        print('Image uploaded successfully');
      } else {
                ...
                children: [
                  ...
                  ListTile(
                    leading: Icon(Icons.work,
                        color: Colors
                            .blue[800]), // Updated icon for Experience
                    title: Text('Experience'),
                    ...
                    onTap: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                            builder: (context) =>
                                MentorExperienceScreen()), // Updated screen
                      );
                    },
                  ),
                  Divider(),
                  ListTile(
                    leading:
                    Icon(Icons.interests, color: Colors.blue[800]),
                    title: Text('Area of Interest'),
                    trailing: Icon(Icons.arrow_forward_ios,
                        color: Colors.blue[800]),
                    onTap: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                            builder: (context) =>
                                AreaOfInterestScreen()), // Updated screen
                      );
                    },
                  ),
                  Divider(),
                  ListTile(
                    ...
                    onTap: () async {
                      bool? result = await Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => MentorEditProfileScreen(
                              firstName: user['first_name'],
                              lastName: user['last_name'],
                              email: user['Email'],
                              phoneNumber: user['PhoneNumber'],
