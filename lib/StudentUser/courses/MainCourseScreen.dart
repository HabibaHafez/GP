...
class _CoursesScreenState extends State<CoursesScreen> {
  ...
  void initState() {
    ...
    futureRecommendedCourses = Future.value([]);
    futureSavedCourses =
        SearchCourseService.fetchAllCourses(); // Use fetchAllCourses
    _loadNationalId();
  }
  ...
  Future<void> _loadNationalId() async {
    ...
    if (id != null) {
      setState(() {
        nationalId = id.toString();
        futureRecommendedCourses =
            RecommendedCourseService.fetchRecommendedCourses(nationalId!);
        futureSavedCourses = SearchCourseService.fetchAllCourses();
      });
    }
  }
  ...
  void _searchCourses() {
    setState(() {
      if (searchController.text.isNotEmpty) {
        futureSavedCourses =
            SearchCourseService.searchCoursesByName(searchController.text);
      } else {
        futureSavedCourses = SearchCourseService.fetchAllCourses();
      }
    });
                ...
                child: FutureBuilder<List<RecommendedCourse>>(
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return Center(
                          child: Text('No recommended courses found'));
                    } else {
                      return ListView(
                        scrollDirection: Axis.horizontal,
                        children: snapshot.data!.map((course) {
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => CourseDetailsScreen(
                                    ...
                                    provider: course.provider,
                                    level: course.level ??
                                        'Unknown', // Use the actual data or a default value
                                    duration: course.duration ??
                                        'Unknown', // Use the actual data or a default value
                                    courseDescription: course.roadMap ??
                                        'No description available', // Use the actual data or a default value
                                    price: course.paid ??
                                        'Free', // Use the actual data or a default value
                                    playUrl: course.link ??
                                        '', // Use the actual data or a default value
                                    courseId: course
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.symmetric(
          vertical: 8.0), // Add vertical space between cards
      child: GestureDetector(
        onTap: onTap,
        child: Container(
          width: double.infinity, // Make the card take the whole row
